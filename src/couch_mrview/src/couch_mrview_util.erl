...
ddoc_to_mrst(DbName, #doc{id=Id, body={Fields}}) ->
    ...
    {ok, IdxState#mrst{sig=couch_hash:hash(term_to_binary(SigInfo))}}.
...
view_sig(Db, State, View, #mrargs{include_docs=true}=Args) ->
    ...
    couch_index_util:hexsig(couch_hash:hash(term_to_binary(Term)));
...
sig_vsn_12x(State) ->
    ...
    couch_hash:hash(term_to_binary(SigData)).
