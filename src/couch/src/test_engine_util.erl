...
gen_write(Engine, St, {create, {DocId, Body, Atts0}}, UpdateSeq) ->
    ...
    Rev = couch_hash:hash(term_to_binary({DocId, Body, Atts})),
...
gen_revision(conflict, DocId, _PrevRev, Body, Atts) ->
    couch_hash:hash(term_to_binary({DocId, Body, Atts}));
gen_revision(delete, DocId, PrevRev, Body, Atts) ->
    gen_revision(update, DocId, PrevRev, Body, Atts);
gen_revision(update, DocId, PrevRev, Body, Atts) ->
    couch_hash:hash(term_to_binary({DocId, PrevRev, Body, Atts})).
...
write_att(Stream, FileName, OrigData, <<>>) ->
    ...
    couch_util:check_md5(Md5, couch_hash:hash(OrigData)),
    Len = size(OrigData),
    couch_att:new([
        {name, FileName},
