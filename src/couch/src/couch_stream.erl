...
foldl(Engine, Md5, UserFun, UserAcc) ->
    InitAcc = {couch_hash:hash_init(), UserFun, UserAcc},
    {Md5Acc, _, OutAcc} = foldl(Engine, fun foldl_md5/2, InitAcc),
    Md5 = couch_hash:hash_final(Md5Acc),
    OutAcc.
...
foldl_md5(Bin, {Md5Acc, UserFun, UserAcc}) ->
    NewMd5Acc = couch_hash:hash_update(Md5Acc, Bin),
...
init({Engine, OpenerPid, OpenerPriority, Options}) ->
    ...
    {ok, #stream{
            ...
            md5=couch_hash:hash_init(),
            identity_md5=couch_hash:hash_init(),
...
handle_call({write, Bin}, _From, Stream) ->
    ...
    if BinSize + BufferLen > Max ->
        ...
        IdenMd5_2 = couch_hash:hash_update(IdenMd5, WriteBin),
        ...
        WriteBin2 ->
            ...
            Md5_2 = couch_hash:hash_update(Md5, WriteBin2)
        end,
...
handle_call(close, _From, Stream) ->
    ...
    IdenMd5Final = couch_hash:hash_final(couch_hash:hash_update(IdenMd5, WriteBin)),
    WriteBin2 = EncodingFun(WriteBin) ++ EndEncodingFun(),
    Md5Final = couch_hash:hash_final(couch_hash:hash_update(Md5, WriteBin2)),
